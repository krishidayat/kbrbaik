...
Route::get('/posts', function () {
    ...
    return view('suara.posts', compact('station', 'categories', 'posts'));
})->name('posts');
...
Route::get('/blog', function () {
    ...
    return view('kbrbaik-blog', compact('station', 'categories', 'posts'));
})->name('kbrbaik.blog');
...
Route::get('/category/{slug}', function ($slug) {
    ...
    $posts = $category->posts()
        ...
        ->latest()
        ->paginate(12);
    return view('suara.category', compact('station', 'posts', 'category'));
})->name('category');
...
Route::get('/musik', function () {
    ...
    return view('musik', compact('station', 'items'));
})->name('musik');
...
Route::get('/komunitas/{slug}', function ($slug) {
    ...
    $communities = \App\Models\Community::where("station_id", $station?->id ?? 1)->where("is_active", true)->get();
    $invitations = \App\Models\Invitation::where("invited_by", auth()->id())->latest()->take(20)->get();
    if ($station && $station->slug === 'kbrbaik') {
        return view('kbrbaik-komunitas-show', compact('station', 'studios', 'communities', 'invitations', 'community'));
    }
    return view('kbrbaik-komunitas-show', compact('station', 'studios', 'communities', 'invitations', 'community'));
})->name('komunitas.show');
...
Route::get('/galeri', function () {
    ...
    $invitations = \App\Models\Invitation::where("invited_by", auth()->id())->latest()->take(20)->get();
    if ($station && $station->slug === 'kbrbaik') {
        return view('kbrbaik-galeri', compact('station', 'items', 'episodes'));
    }
    return view('galeri', compact('station', 'items', 'episodes'));
})->name('galeri');
...
Route::get('/', function () {
    ...
    return view('home', compact('station', 'schedules', 'episodes', 'latestPosts'));
})->name('home');
...
Route::get('/kredensi/tulis', function () {
    ...
    $station = request('station');
    $categories = \App\Models\\App\Models\Category::where('station_id', $station?->id ?? 1)->where('group', 'kbrbaik')->get();
    $studios = \App\Models\Studio::where('station_id', $station?->id ?? 1)->where('is_active', true)->get();
    $communities = \App\Models\Community::where("station_id", $station?->id ?? 1)->where("is_active", true)->get();
    $invitations = \App\Models\Invitation::where("invited_by", auth()->id())->latest()->take(20)->get();
    $posts = \App\Models\\App\Models\Post::where('author', auth()->user()->name)->orWhere('created_by', auth()->id())->latest()->take(10)->get();
    return view('kredensi-dashboard', compact('station'));
})->name('kredensi.tulis');
