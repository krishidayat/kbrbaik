...
@section('title', 'PGIS - ' . ($station?->name ?? 'Suara PGIW Jabar'))

@section('content')
<div class="max-w-6xl mx-auto px-4 py-8">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold">PGIS</h1>
            <p class="text-gray-500 mt-1">Persekutuan Gereja Indonesia Setempat se-Jawa Barat</p>
        </div>
        <a href="{{ route('struktur.pgis') }}" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition flex-shrink-0">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            Struktur &amp; Personalia
        </a>
    </div>

    <form method="GET" action="{{ route('pgis') }}" class="mb-8">
        <select name="category" onchange="this.form.submit()" class="w-full sm:w-72 px-4 py-2.5 rounded-lg border border-gray-300 bg-white text-sm text-gray-700 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            <option value="">Semua PGIS</option>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <article class="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition">
            ...
            @else
            <div class="w-full h-28 bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center text-2xl">📰</div>
            @endif
            <div class="p-3">
                @if ($post->category)
