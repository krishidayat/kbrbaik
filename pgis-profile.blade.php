...
@section('content')
@php
$photos = \App\Models\GalleryItem::where('album', 'pgis-' . $item->slug)
    ->where('is_active', true)->latest()->take(6)->get();
$bidangs = \App\Models\Category::where('group', 'bidang')->whereNull('parent_id')->where('station_id', 2)->get();
$strukturOrder = ['Ketua', 'Sekretaris', 'Bendahara'];
@endphp

<div class="relative overflow-hidden bg-gradient-to-br from-primary-500 to-primary-700 text-white">
    <div class="max-w-6xl mx-auto px-4 py-12 md:py-16">
        <a href="{{ route('pgis') }}" class="inline-flex items-center gap-1 text-sm text-white/70 hover:text-white transition mb-4">
        ...
        <div class="flex items-start justify-between">
            <div>
                ...
                <p class="text-white/80 mt-2 max-w-xl">Persekutuan Gereja-gereja di Indonesia Barat</p>
            </div>
            @auth
...
<div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
    ...
    <section class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        <div class="bg-gradient-to-r from-primary-600 to-primary-700 text-white px-6 py-4">
            <h2 class="text-xl font-bold">🏛️ Struktur Kepengurusan</h2>
            <p class="text-sm text-white/70 mt-0.5">Periode {{ date('Y') - 1 }}-{{ date('Y') + 2 }}</p>
        </div>
        <div class="p-6">
    ...
    <section class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        <div class="bg-gradient-to-r from-primary-600 to-primary-700 text-white px-6 py-4">
            <h2 class="text-xl font-bold">📋 Bidang & Komisi</h2>
            <p class="text-sm text-white/70 mt-0.5">Program kerja PGIS {{ $item->name }}</p>
        </div>
        <div class="p-6">
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 hover:shadow-sm transition">
                    ...
                    @php $subs = \App\Models\Category::where('parent_id', $b->id)->get(); @endphp
                    @if ($subs->count())
                    <ul class="mt-2 space-y-1">
                        @foreach ($subs as $sub)
                        <li class="text-xs text-gray-500 flex items-center gap-1.5">
                            <svg class="w-3 h-3 text-primary-500 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                            {{ $sub->name }}
                        </li>
                        @endforeach
                    </ul>
    ...
    <section class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        <div class="bg-gradient-to-r from-primary-600 to-primary-700 text-white px-6 py-4 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-white/70 mt-0.5">Dokumentasi kegiatan {{ $item->name }}</p>
            </div>
            <a href="{{ route('galeri') }}?album=pgis-{{ $item->slug }}" class="text-sm text-white/70 hover:text-white transition">Lihat Semua →</a>
        </div>
        <div class="p-6">
            @if ($photos->isEmpty())
            <div class="text-center py-8 text-gray-400">
                <svg class="w-12 h-12 mx-auto mb-3 text-gray-300" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z"/></svg>
                <p class="text-sm mt-2">Belum ada foto. Upload foto dengan album <strong>pgis-{{ $item->slug }}</strong> di galeri.</p>
            </div>
            @else
