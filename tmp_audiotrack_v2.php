...
namespace AppFilamentResourcesAudioTracks;

use AppFilamentResourcesAudioTracksAudioTrackResource;
use AppModelsAudioTrack;
use BackedEnum;
...
class AudioTrackResource extends Resource
{
        ...
        return $schema
            ->components([
                ...
                FileUpload::make('file_path')
                    ->label('Audio File')
                    ->disk('audio')
                    ->directory(fn ($get) => ($get('community_id') ? AppModelsCommunity::find($get('community_id'))?->slug : 'unknown') . '/' . ($get('playlist') ?? 'siang'))
    ...
    {
        return ['index' => PagesListAudioTracks::route('/')];
    }
